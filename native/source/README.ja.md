...
最初のソーススナップショットは、デスクトップアプリの `db/cassandra.rs` からコピーされました。

ソース SHA-256: `8609b051e3c1d9c890b613266d04bb25a7350fcab8a0b81c64bc530ebc269fa9`。


このディレクトリは、`irodori.cassandra` のマイグレーションステージングエリアです。アクティブなネイティブエントリポイントは `src/lib.rs` にあり、共有ABIヘルパーは `src/abi.rs` にあり、エンジンの動作は `src/stub.rs` または `src/driver.rs` にあります。エンジン固有の接続/クエリ/メタデータコードは、これらのスナップショットからその動作モジュールに移動されるべきです。これは、コネクタのランタイム契約がデスクトップアプリに組み込まれるためです。

## マイグレーションスナップショット

| 種類 | ソース | 目的地 | SHA-256 |
|---|---|---|---|
| `desktop-db-adapter` | `apps/desktop/src-tauri/src/db/cassandra.rs` | `native/source/irodori-table/apps/desktop/src-tauri/src/db/cassandra.rs` | `8609b051e3c1d9c890b613266d04bb25a7350fcab8a0b81c64bc530ebc269fa9` |
...
エンジンの状態は `knowledge/engines.json`: `wired` です。